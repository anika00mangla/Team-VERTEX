...
# Add this palette at the top of visualize[1].py
COLOR_PALETTE = {
...
class MaskDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            ...
            mask = transforms.functional.resize(mask, image.shape[1:], interpolation=transforms.InterpolationMode.NEAREST)    ###changed 
            mask = torch.from_numpy(np.array(mask)).long()                                   ### changed
...
class SegmentationHeadConvNeXt(nn.Module):
    def __init__(self, in_channels, out_channels, tokenW, tokenH):
        ...
        self.stem = nn.Sequential(
            ...
            nn.BatchNorm2d(128),  # <--- INSERTED HERE
            nn.GELU()
        )                                                               ####changed 
...
def compute_iou(pred, target, num_classes=10, ignore_index=255):
    ...
    for class_id in range(num_classes):
        ...
        if union == 0:
            ...
            iou_per_class.append((intersection / (union + 1e-7)).cpu().numpy())     ###changed
...
def evaluate_metrics(model, backbone, data_loader, device, num_classes=10, show_progress=True):
    ...
    with torch.no_grad():
        for imgs, labels in loader:
            ...
            labels = F.interpolate(labels.unsqueeze(1).float(), size=imgs.shape[2:], mode='nearest').long().squeeze(1)
...
def main():
    ...
    # Loss and optimizer
    weights = torch.tensor([2.0, 1.0, 2.0, 1.0, 1.0, 1.5, 3.0, 1.0, 1.0, 0.5]).to(device)        ###changed
    loss_fct = torch.nn.CrossEntropyLoss(weight=weights)                                           ###changed
    ...
    for epoch in epoch_pbar:
        ...
        for imgs, labels in train_pbar:
            ...
            labels = F.interpolate(labels.unsqueeze(1).float(), size=imgs.shape[2:], mode='nearest').long().squeeze(1)           ####changed
